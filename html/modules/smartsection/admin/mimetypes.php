...
/**
* $Id: mimetypes.php,v 1.1 2012/03/31 09:54:07 ohwada Exp $
* Module: SmartSection
...
$aSortBy = array('mime_id' => _AM_SSECTION_MIME_ID, 'mime_name' => _AM_SSECTION_MIME_NAME, 'mime_ext' => _AM_SSECTION_MIME_EXT,
                 'mime_admin' => _AM_SSECTION_MIME_ADMIN, 'mime_user' => _AM_SSECTION_MIME_USER);
$aOrderBy = array('ASC' => _AM_SSECTION_TEXT_ASCENDING, 'DESC' => _AM_SSECTION_TEXT_DESCENDING);
$aLimitBy = array('10' => 10, '15' => 15, '20' => 20, '25' => 25, '50' => 50, '100' => 100);
$aSearchBy = array('mime_id' => _AM_SSECTION_MIME_ID, 'mime_name' => _AM_SSECTION_MIME_NAME, 'mime_ext' => _AM_SSECTION_MIME_EXT);
...
function add()
{
    ...
    if(!isset($_POST['add_mime'])){
        ...
        smartsection_adminMenu(4, _AM_SSECTION_MIMETYPES);
        
        smartsection_collapsableBar('mimemaddtable', 'mimeaddicon', _AM_SSECTION_MIME_ADD_TITLE);
        
        $session = SmartsectionSession::singleton();
        $mime_type = $session->get('smartsection_addMime');
        $mime_errors = $session->get('smartsection_addMimeErr');
        
        //Display any form errors
        if (! $mime_errors === false) {
            smartsection_renderErrors($mime_errors, smartsection_makeURI(SMARTSECTION_ADMIN_URL.'mimetypes.php', array('op'=>'clearAddSession')));
        }
        ...
        echo "<tr valign='top'>
                  <td class='head'>"._AM_SSECTION_MIME_USERF."</td>
                  <td class='even'>";
        ...
        echo "<tr valign='top'>
                  <td class='head'></td>
                  <td class='even'>
                      <input type='submit' name='add_mime' id='add_mime' value='"._AM_SSECTION_BUTTON_SUBMIT."' class='formButton' />
                      <input type='button' name='cancel' value='"._AM_SSECTION_BUTTON_CANCEL."' onclick='history.go(-1)' class='formButton' />
                  </td>
              </tr>";
        echo "</table></form>";
        // end of add form
        ...
        echo "<table width='100%' cellspacing='1' class='outer'>";
        echo "<tr><th colspan='2'>"._AM_SSECTION_MIME_FINDMIMETYPE."</th></tr>";
        
        echo "<tr class='foot'>
                  <td colspan='2'><input type='submit' name='find_mime' id='find_mime' value='"._AM_SSECTION_MIME_FINDIT."' class='formButton' /></td>
              </tr>";
        
        echo "</table></form>";
        
        smartsection_close_collapsable('mimeaddtable', 'mimeaddicon');
        ...
        if (strlen(trim($mime_ext)) == 0) {
            $has_errors = true;
            $error['mime_ext'][] = _AM_SSECTION_VALID_ERR_MIME_EXT;
        }
        
        if (strlen(trim($mime_name)) == 0) {
            $has_errors = true;
            $error['mime_name'][] = _AM_SSECTION_VALID_ERR_MIME_NAME;
        }
        
        if (strlen(trim($mime_types)) == 0) {
            $has_errors = true;
            $error['mime_types'][] = _AM_SSECTION_VALID_ERR_MIME_TYPES;
        }
        ...
        if ($has_errors) {
            ...
            $session->set('smartsection_addMime', $mime);
            $session->set('smartsection_addMimeErr', $error);
        ...
        if(!$hMime->insert($mimetype)){
            redirect_header(SMARTSECTION_ADMIN_URL."/mimetypes.php?op=manage&limit=$limit&start=$start", 3, _AM_SSECTION_MESSAGE_ADD_MIME_ERROR);
        } else {
            _clearAddSessionVars();
            header("Location: ".SMARTSECTION_ADMIN_URL."/mimetypes.php?op=manage&limit=$limit&start=$start");
        }
    }
}
...
function delete()
{
    ...
    if(!isset($_REQUEST['id'])){
        redirect_header(SMARTSECTION_ADMIN_URL."/mimetypes.php", 3, _AM_SSECTION_MESSAGE_NO_ID);
    } else {
        $mime_id = intval($_REQUEST['id']);
    }
    $mimetype =& $hMime->get($mime_id);     // Retrieve mimetype object
    if(!$hMime->delete($mimetype, true)){
        redirect_header(SMARTSECTION_ADMIN_URL."/mimetypes.php?op=manage&id=$mime_id&limit=$limit&start=$start", 3, _AM_SSECTION_MESSAGE_DELETE_MIME_ERROR);
    } else {
        header("Location: ".SMARTSECTION_ADMIN_URL."/mimetypes.php?op=manage&limit=$limit&start=$start");
    }
}
...
function edit()
{
    ...
    if(!isset($_POST['edit_mime'])){
        $session = SmartsectionSession::singleton();
        $mime_type = $session->get("smartsection_editMime_$mime_id");
        $mime_errors = $session->get("smartsection_editMimeErr_$mime_id");
        
        // Display header
        smartsection_xoops_cp_header();
        smartsection_adminMenu(4, _AM_SSECTION_MIMETYPES . " > " . _AM_SSECTION_BUTTON_EDIT);
        
        smartsection_collapsableBar('mimemedittable', 'mimeediticon', _AM_SSECTION_MIME_EDIT_TITLE);

        //Display any form errors
        if (! $mime_errors === false) {
            smartsection_renderErrors($mime_errors, smartsection_makeURI(SMARTSECTION_ADMIN_URL.'/mimetypes.php', array('op'=>'clearEditSession', 'id'=>$mime_id)));
        }
        ...
        echo "<table width='100%' cellspacing='1' class='outer'>";
        echo "<tr><th colspan='2'>"._AM_SSECTION_MIME_MODIFYF."</th></tr>";
        echo "<tr valign='top'>
                  <td class='head'>"._AM_SSECTION_MIME_EXTF."</td>
                  <td class='even'><input type='text' name='mime_ext' id='mime_ext' value='$mime_ext' size='5' /></td>
              </tr>";
        ...
        echo "<tr valign='top'>
                  <td class='head'></td>
                  <td class='even'>
                      <input type='submit' name='edit_mime' id='edit_mime' value='"._AM_SSECTION_BUTTON_UPDATE."' class='formButton' />
                      <input type='button' name='cancel' value='"._AM_SSECTION_BUTTON_CANCEL."' onclick='history.go(-1)' class='formButton' />
                  </td>
              </tr>";
        echo "</table></form>";
        // end of edit form
        smartsection_close_collapsable('mimeedittable', 'mimeediticon');
        xoops_cp_footer();
    } else {
        $mime_admin = 0;
        $mime_user = 0;
        ...
        if ($has_errors) {
            ...
            $mime['mime_admin'] = $mime_admin;
            $mime['mime_user'] = $mime_user;
            $session->set('smartsection_editMime_'. $mime_id, $mime);
            $session->set('smartsection_editMimeErr_'. $mime_id, $error);
            header('Location: '. smartsection_makeURI(SMARTSECTION_ADMIN_URL.'/mimetypes.php', array('op'=>'edit', 'id'=>$mime_id), false));
        }
        ...
        if(!$hMime->insert($mimetype, true)){
            redirect_header(SMARTSECTION_ADMIN_URL."/mimetypes.php?op=edit&id=$mime_id", 3, _AM_SSECTION_MESSAGE_EDIT_MIME_ERROR);
        } else {
...
function manage()
{
    ...
    if(isset($_POST['mime_search'])){
        header("Location: ".SMARTSECTION_ADMIN_URL."/mimetypes.php?op=search");
        exit();
    }
    
    smartsection_xoops_cp_header();
    smartsection_adminMenu(4, _AM_SSECTION_MIMETYPES);
    smartsection_collapsableBar('mimemanagetable', 'mimemanageicon', _AM_SSECTION_MIME_MANAGE_TITLE, _AM_SSECTION_MIME_INFOTEXT);
    ...
    echo "</table>";
    echo "</td></tr>";
    echo "<tr><th colspan='6'>"._AM_SSECTION_MIME_MANAGE_TITLE."</th></tr>";
    echo "<tr class='head'>
    ...
    foreach($mimetypes as $mime){
        echo "<tr class='even'>
                  ...
                  <td>
                      <a href='".SMARTSECTION_ADMIN_URL."/mimetypes.php?op=updateMimeValue&amp;id=".$mime->getVar('mime_id')."&amp;mime_user=".$mime->getVar('mime_user')."&amp;limit=".$limit."&amp;start=".$start."'>
    ...
    smartsection_close_collapsable('mimemanagetable', 'mimemanageicon');
...
function updateMimeValue()
{
    ...
    if($hMime->insert($mimetype, true)){
        ...
        redirect_header(SMARTSECTION_ADMIN_URL."/mimetypes.php?limit=$limit&start=$start", 3);
    }
}
...
{
    $session = SmartsectionSession::singleton();
    $session->del('smartsection_addMime');
    $session->del('smartsection_addMimeErr');
}
